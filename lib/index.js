...
module.exports = {
    convert: function (request, options, callback) {
        ...
        var indent, trim, headersData, body, text, redirect, timeout, multiLine, format, snippet, testMode;
        ...
        testMode = options.testMode ? options.testMode : false;

        snippet = testMode ? `curl ${form('-s', format)}` : 'curl';
        if (redirect) {
        ...
        if (multiLine) {
            ...
            indent = ' \\\n' + indent.repeat(options.indentCount || (options.indentType === 'tab' ? 1 : 4));
        }
        else {
            indent = ' ';
        }
